...
void Graph::AddNode(int cIdx, int pIdx, int eShape)
{
  ...
  if(!eShape)
  {
    switch(IdentLoc(pIdx, cIdx))
    {
      case 0: nodes[pIdx].S.push_back(cIdx); SortS(nodes[pIdx]);
              nodes[cIdx].E.push_back(pIdx); SortE(nodes[cIdx]);
             break;
      case 1: nodes[pIdx].S.push_back(cIdx); SortS(nodes[pIdx]);
              nodes[cIdx].W.push_back(pIdx); SortW(nodes[cIdx]);
             break;
      case 2: nodes[pIdx].S.push_back(cIdx); SortS(nodes[pIdx]);
              nodes[cIdx].N.push_back(pIdx); SortN(nodes[cIdx]);
             break;
      case 3: nodes[pIdx].N.push_back(cIdx); SortN(nodes[pIdx]);
              nodes[cIdx].E.push_back(pIdx); SortE(nodes[cIdx]);
             break;
      case 4: nodes[pIdx].N.push_back(cIdx); SortN(nodes[pIdx]);
              nodes[cIdx].W.push_back(pIdx); SortW(nodes[cIdx]);
             break;
      case 5: nodes[pIdx].N.push_back(cIdx); SortN(nodes[pIdx]);
              nodes[cIdx].S.push_back(pIdx); SortS(nodes[cIdx]);
             break;
      case 6: nodes[pIdx].W.push_back(cIdx); SortW(nodes[pIdx]);
              nodes[cIdx].E.push_back(pIdx); SortE(nodes[cIdx]);
             break;
      case 7: nodes[pIdx].E.push_back(cIdx); SortE(nodes[pIdx]);
              nodes[cIdx].W.push_back(pIdx); SortW(nodes[cIdx]);
             break;
    }
  }
  else
  {
    switch(IdentLoc(pIdx, cIdx))
    {
      case 0: nodes[pIdx].W.push_back(cIdx); SortW(nodes[pIdx]);
              nodes[cIdx].N.push_back(pIdx); SortN(nodes[cIdx]);
             break;
      case 1: nodes[pIdx].E.push_back(cIdx); SortE(nodes[pIdx]);
              nodes[cIdx].N.push_back(pIdx); SortN(nodes[cIdx]);
             break;
      case 2: nodes[pIdx].S.push_back(cIdx); SortS(nodes[pIdx]);
              nodes[cIdx].N.push_back(pIdx); SortN(nodes[cIdx]);
             break;
      case 3: nodes[pIdx].W.push_back(cIdx); SortW(nodes[pIdx]);
              nodes[cIdx].S.push_back(pIdx); SortS(nodes[cIdx]);
             break;
      case 4: nodes[pIdx].E.push_back(cIdx); SortE(nodes[pIdx]);
              nodes[cIdx].S.push_back(pIdx); SortS(nodes[cIdx]);
             break;
      case 5: nodes[pIdx].N.push_back(cIdx); SortN(nodes[pIdx]);
              nodes[cIdx].S.push_back(pIdx); SortS(nodes[cIdx]);
...
bool Graph::buildNearestNeighbors_single_node(unsigned num_terms, unsigned node_idx) {
	...
	unsigned idx = 0;
	for(unsigned abc=0;abc<sorted.size();abc++) if(sorted[abc] == node_idx) { idx = abc; break;}
...
unsigned Graph::calc_tree_pl()
{
	...
	for(unsigned j = 0;  j < orig_num_terminals;  ++j ) 
	{
        ...
        cout << "Child = " << child << " ; SV Par = " << par << endl; 
			...
			child = par; par = nodes[par].parent;
		}
	}
	return (pl);
}
...
void Graph::PDBU_new_NN() {
  ...
  while ((tree_cost_difference > 0) || (tree_cost_difference < -1)) {
    ...
    for(unsigned j = 0;  j < num_terminals;  ++j ) /* For each terminal */
    {
    	...
    	unsigned iter = 0;
    	vector<int> tmp; nodes[j].swap_space.push_back(tmp); 
      tmp.clear();
      ...
      {
    		vector<int> tmp; nodes[j].swap_space.push_back(tmp); tmp.clear();
      ...
      //For every row in the nodes[j].swap_space
        ...
        //For every element in the "iter"th row of nodes[j].swap_space
          ...
          //Node to get new edge
              ...
              overall_min_nn_idx = oct; overall_swap_dist = iter; 
              overall_initial_i = j;
            }
    			}
    		}
...
bool Graph::doSteiner_HoVW() {
  ...
  for(int k=tree_struct.size()-3;k>=0;k--)  //Starting from the nodes in the second level from bottom
  {
  	for (unsigned l=0;l<tree_struct[k].size();l++)
  	{
  		...
  		set_of_nodes.push_back(tmp_node); set_of_nodes.push_back(nodes[par]);
  ...
  if(verbose > 2)
  {
	...
	for(int k=1;k<tree_struct_1darr.size();k++) {
		...
		if(cnode_dc > 0) {
			...
			if(use_nn ==1) {size = nn[cnode].size();}
			else {size = k; }

			for(unsigned ag=0;ag<size;ag++) {
				if(use_nn == 1) {nn2 = nn[cnode][ag];}
				else {nn2 = tree_struct_1darr[ag];}
...
bool Graph::get_overlap_lshape(vector<Node>& set_of_nodes, int index) {
  ...
  tmp1.push_back(0); tmp1.push_back(1);
  ...
  unsigned max_ov = 0; vector<int> best_config; vector<Node> best_sps_x, best_sps_y;
  ...
  for(unsigned i=0;i<result.size();i++)
  {
  	...
  	{	
        ...
        tmp4.push_back(set_of_nodes[j]);
  			set_of_points.push_back(tmp4); tmp4.clear();
        ...
        tmp5.push_back(set_of_nodes[j]);
  			set_of_points.push_back(tmp5); tmp5.clear();
  	...
  	if(lower_ov >= max_ov)
  	{
  		max_ov = lower_ov;
  		best_config = config;
  		best_sps_x.clear(); best_sps_y.clear();
  		best_sps_curr_node_idx_x = 9999999; best_sps_curr_node_idx_y = 9999999;
  		if (set_of_points.size() != num_edges)
  		{
  			best_sps_x = set_of_points[num_edges]; best_sps_curr_node_idx_x = nodes[index].idx_of_cn_x;
  			best_sps_y = set_of_points[num_edges+1]; best_sps_curr_node_idx_y = nodes[index].idx_of_cn_y;
  		} 
  	}
  	for (unsigned i = 0; i < set_of_points.size(); ++i) set_of_points[i].clear();
  	set_of_points.clear(); config.clear();
  }
  ...
  for (unsigned p=0;p<result.size();p++) { 
      ...
      //Set first row which matches as reference row
  		if (max_ap_cnt == 1) {tmp_res = result[p];} 
  		else if (max_ap_cnt > 1) { 
  			for (unsigned idk=0;idk<res_size-1;idk++) {
  				if (res_size == 2) { not_dont_care_flag++; }
  				else {
  					if (result[p][idk] == tmp_res[idk]) { 
  						not_dont_care_flag++; not_dont_care_child.push_back(idk);
  					}
  				}
  			}
  		}
  ...
  if ((dont_care_flag !=0) && (dont_care_flag == max_ap_cnt - 1)) {
  	...
  	dont_care_child.clear(); List1.clear();
  }
  tmp_res.clear(); not_dont_care_child.clear();
  ...
  best_config.clear(); best_sps_x.clear(); best_sps_y.clear();
  max_ov = 0;
  for(unsigned i=0;i<result.size();i++)
  {
    vector < vector<Node> > set_of_points;
  ...
  res_size = result[0].size() ; not_dont_care_flag = 0;
  for (unsigned p=0;p<result.size();p++) { 
  	if (max_ov == result[p][res_size-1]) {
  		max_ap_cnt++;
  		if (max_ap_cnt == 1) {tmp_res = result[p];}
  ...
  lists.clear(); result.clear(); tmp1.clear(); tmp2.clear();
  set_of_nodes.clear();
...
unsigned Graph::calc_overlap(vector < vector<Node> > &set_of_nodes) {
	...
	for (int i=0;i<set_of_nodes.size();i++)
	{
		for(unsigned j=i+1;j<set_of_nodes.size();j++)
		{
           ...
           boost::lexical_cast<string>(m[2].y) + ")";
			if(verbose>2) {	
        cout << "String1 = " << s0 << endl; cout << "String2 = " << s1 << endl; 
      }
			boost::geometry::read_wkt(s0, line1); boost::geometry::read_wkt(s1, line2);
			...
			if(output_nodes.size() > 1)
			{
				for (unsigned k = 0; k < output_nodes.size(); ++k) 
				{
					...
					if((output_nodes[k].x==curr_node.x)&&(output_nodes[k].y==curr_node.y)) { curr_node_flag = 1; }
					//If not present in all_pts
					bool is_present = false; unsigned all_pts_idx = 9999999;
					for(unsigned s=0;s<all_pts.size();s++) {
						if((output_nodes[k].x==all_pts[s].x)&&(output_nodes[k].y==all_pts[s].y)) {
							if (curr_node_flag == 1) { 
                is_present = true; all_pts_idx = s; 
			...
			output.clear(); n.clear(); m.clear(); output_nodes.clear();
		}
	}
	if(all_pts.size() > 1)
	{
		unsigned posn_of_cn = 9999999;
		for (unsigned u=0;u<all_pts.size();u++) if((all_pts[u].x == curr_node.x) && (all_pts[u].y == curr_node.y)) {posn_of_cn = u; break;}
    ...
    sort(sorted_y.begin(),sorted_y.end(),comp_y);
      ...
      if((sorted_x[u].x == curr_node.x) && (sorted_x[u].y == curr_node.y)) {
        nodes[curr_node.idx].idx_of_cn_x = u; break;
      }
		for (unsigned u=0;u<sorted_y.size();u++) 
      if((sorted_y[u].x == curr_node.x) && (sorted_y[u].y == curr_node.y)) {
        nodes[curr_node.idx].idx_of_cn_y = u; break;
      }
		set_of_nodes.push_back(sorted_x); set_of_nodes.push_back(sorted_y);
...
unsigned Graph::calc_ov_x_or_y(vector<Node>& sorted, Node curr_node, char tag[]) {
	...
	for (int i=0;i<sorted.size();i++) //Getting position of "curr_node" 
		if((sorted[i].x == curr_node.x) && (sorted[i].y == curr_node.y)) {ind_of_curr_node = i; break;}
	if (ind_of_curr_node > 0) {
		...
		cnt = 0; unsigned s = tmp.size(); //cout << "Size of tmp = " << s << endl;
...
void Graph::get_level_in_tree()
{
	tree_struct.clear();
	unsigned iter=0;
	vector<int> tmp1; tree_struct.push_back(tmp1); tmp1.clear();
	tree_struct[iter].push_back(0);
	iter++;
	unsigned j = 0, level_count = 0;
	vector<int> set_of_chi = nodes[j].children, tmp;
	vector<int> tmp2; tree_struct.push_back(tmp2); tmp2.clear();
